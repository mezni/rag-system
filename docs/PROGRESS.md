...
| 1.7 | | `source_id` is the canonical *absolute* path. A container run (`/rag-system/...`) and a host run (`/home/dali/WORK/rag-system/...`) address the same file under different keys, so each re-scan deactivates the other's document versions. | Normalize `source_id` to a path relative to the scan root (with a configurable mount anchor) so host and container runs converge on the same key and stop toggling each other's versions. | Planned: derive `source_id` relative to the scan root in `discover_files`/`diff_against_store`; align existing rows via migration/backfill; re-verify host + container convergence. | in progress |